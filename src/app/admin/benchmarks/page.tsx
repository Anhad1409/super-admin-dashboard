  ...
  return (
    <div className="mx-auto max-w-[1300px] space-y-5">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatTile icon={ArrowUp} label="Top performer" value={topPerformer.client.name.split(" ")[0]} sub="highest blended percentile" tint="var(--color-success)" />
        <StatTile icon={ArrowDown} label="Needs attention" value={laggard.client.name.split(" ")[0]} sub="lowest blended percentile" tint="var(--color-danger)" />
        <StatTile icon={Gauge} label="Median connect" value={`${benchMedians.connect}%`} sub="platform middle" tint="var(--color-mango)" />
        <StatTile icon={BarChart3} label="Median health" value={benchMedians.health} sub="platform middle" tint="var(--color-steam)" />
      </div>
